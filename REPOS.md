...
| [drydock-survey](https://github.com/j0yen/drydock-survey) | `drydock-survey` | Normalized drift inventory: aggregates `binstale fleet --format json`, `adopt verify --format json`, and a capability-based kernel-pkg staleness probe into one ranked JSON muster. Read-only; exit 0 even when sources fail (sentinel emitted). Foundation for the drydock-classify/apply/ledger fleet. |
