...
| [mcphost-python-client](https://github.com/j0yen/mcphost-python-client) | `mcphost` | `pip install mcphost` — thin Python client + CLI over the hosted mcphost MCP server (`signup`/`test`/`publish`/`call`/`list`/`usage`, every command a 1:1 map to an existing server tool, no MCP JSON-RPC hand-writing required); `clientInfo={name:"mcphost-python"}` on every handshake for funnel attribution. |
...
| [fleet-readonly-bridge](https://github.com/j0yen/fleet-readonly-bridge) | `fleet-readonly-bridge` (Python) | Local, read-only HTTP service on the hub turning fleet-sync's `check --json` into a bounded, schema-versioned snapshot (`GET /v1/snapshot`, `GET /healthz`); every write method/mutation route is rejected before anything runs, binds `127.0.0.1` by default, redacts token-like values, 256 KiB response cap. Plumbing for the MCPHost fleet adapter (see `visions/mcphost-fleet-infrastructure.md`). |
| [mcphost-fleet-adapter](https://github.com/j0yen/mcphost-fleet-adapter) | (Python, MCPHost tool) | Bounded MCPHost `python`-kind tool over fleet-readonly-bridge's snapshot: fixed HTTPS upstream + MCPHost-managed bearer secret (`SECRET_FLEET_BRIDGE_TOKEN`), 10 s timeout, 256 KiB cap fails closed, preserves `unreachable`/`unknown` node states distinctly from `ok`, marks stale snapshots, never accepts a caller-supplied url/method/headers, never leaks the secret (including from `http.client`'s own header-validation exceptions — closed by an independent-review round). Consumes `fleet-readonly-bridge`; see `visions/mcphost-fleet-infrastructure.md`. |
...
| [wintermute-agent](https://github.com/j0yen/wintermute-agent) | — (shell + plugin) | Hermes config-as-code + idempotent `deploy.sh`/`doctor.sh` that make Wintermute Hub a fleet member: key-owned reversible merge into `~/.hermes/config.yaml` (refuses on hand-edit drift unless `--take-over`), RedBaron-built binaries shipped and verified-executable before install (glibc-mismatch-safe — RedBaron's 2.43 vs the Hub's 2.39 bit `recall` for real), the `notes`/`PRDs` clones, and `doctor` reporting one line per check (bin/clone/config/hermes-version/gateway-unit/ssh-reach) the way `fleet-sync check` does for skills. Also ships a hermes plugin (`plugins/fleet/wintermute_fleet/`, pushed to `~/.hermes/plugins/`) registering fleet actions — `fleet_status`, `node_run`, `build_queue`, `burst_status`, `probe`, `bus_publish`, `bus_peers`, `recall_query`, `recall_write`, `summa_ask`, `summa_page` — as eleven typed, tiered tools, plus three `on_session_start` hooks (FLEET.md names, agorabus peers/intents, recall memories) shared with the Claude Code SessionStart hooks. Now also ships the governance ledger (`ledger/wintermute_ledger/`, PRD-wintermute-governance-ledger): `pre_tool_call`/`post_tool_call` hooks that classify, approve, and ledger every tool call (plugin tools and raw terminal alike) into an append-only NDJSON log under `config/policy.yaml`'s tier-per-tool + command-class policy data, redacting credentials before anything is written; `wintermute ledger`/`wintermute policy test` CLI and a `doctor.sh` policy-hash drift check round it out. Now also ships the shared-memory sync (`sync/wintermute_memory_sync/`, PRD-wintermute-shared-memory): `wintermute-memory-sync run|doctor` pulls the memory repo, reindexes recall/summa only on change, writes `.CONFLICT` sidecars instead of ever dropping a version on a rebase conflict, and secret-scans every commit (a credential-shaped hit excludes only that file, the rest still push). Also ships the `memory.provider: recall` hermes plugin (`plugins/memory/wintermute_memory/`) — top-k recall reads and provenance-stamped (`origin: wintermute` + session id) recall writes under a `wintermute/` subject prefix. Now also ships bus wake (`bus/wintermute_bus/`, PRD-wintermute-bus-wake): a Hub-side bridge that turns fleet-bus events (`fleet.beacon.*`, `build.archive`, `build.prd.*`, `build.verdict`, `build.burst.*`, `build.stall`, `build.quota`) into hermes webhook activations with dedupe/backoff/drop-counting and quiet-hours + loop-guard routing off `config/wake.yaml`, a publisher that puts the RedBaron hawk probe's deltas on the bus as `build.<kind>` events instead of leaving them as script stdout, Wintermute's own `agent.activity`/intent bus presence, and a `wintermute peer run/status/stop` CLI over hermes' Runs API (`peer-contract.md`). Now also ships patrol (`patrol/wintermute_patrol/`, PRD-wintermute-patrol): seven playbook skills (`skills/fleet/burst-ttl`, `build-stall` + its `wedge-sccache-orphans` sub-playbook, `burst-no-session`, `disk-pressure`, `quota-saturated`, `probe-verdict-flip`, `gate-block-verdict`, the last observe-only) and five doctrine skills (`five-whys-before-fixing`, `verify-before-concluding`, `three-state-probes`, `no-pattern-kills`, `never-hand-build`) turn tonight's one-off incident judgment into durable, three-state (nominal/handled/escalated) diagnosis-then-act sequences; an action guard keeps every playbook at or below `act_reversible` (a step shaped like `hcloud server delete` never executes, only escalates with the proposed command); a 15-minute cron sweep (`wintermute patrol sweep`) and per-event dispatch (`wintermute patrol run --playbook <name>`) write one ledger row per run through the existing `LedgerStore` and publish `agent.activity` on `handled`/`escalated`; Telegram escalations are ≤12 lines with a ≤5-row evidence table and deduped per incident id for 60 minutes. |
