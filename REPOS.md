...
| [doxa](https://github.com/j0yen/doxa) | `doxa` | v0.2.0 — Framework-neutral BFO-grounded moral TBox for comparative ethics: 15 moral-domain classes + 8 object properties compiled to OWL 2 DL. Three normative framework modules (consequentialism, deontology, virtue-ethics) as auditable TOML axiom files with philosophicalGrounding annotations citing Mill, Kant, and Aristotle. `doxa build-core`/`doxa check-core`/`doxa list [--format json]`/`doxa build <framework> [--all]` subcommands. |
...
| [mqo-demo-runner](https://github.com/j0yen/mqo-demo-runner) | `mqo-demo-runner` | Orchestrates the five-pillar AtScale AI story end-to-end as one provable transcript: calls catalog-embed, binding-confidence, clarify, time-intelligence, engine-parity, sensitivity-scan, ousia-grounding, and rosetta-credential in order; `run --mock` works with no sibling binaries for CI; `--format md` renders a human demo script; `serve` exposes the pipeline as an MCP tool. |
| [mqo-trace-harvest](https://github.com/j0yen/mqo-trace-harvest) | `mqo-trace-harvest` | Harvest NL→MQO candidate pairs from mqo-demo-runner transcripts and mqo-bench run logs for human-gated golden-set growth: `extract` pulls `{nl_question, bound_mqo}` pairs with provenance, `dedupe` removes near-duplicates against an existing golden set (trigram cosine), `review` emits a human-review markdown worksheet. Hard invariant: no subcommand writes into a golden-set file — promotion is a separate human-gated step. |
